...
// --- [ระบบคำนวณย้อนหลังที่ปรับปรุงแล้ว] ---
    ...
    if (lastExit && lastExit !== "undefined") {
        ...
        if (diffSeconds > 0) {
            ...
            if (isActuallySwitched) {
                ...
                // กรณีจอดับ: ไม่หักพลังงาน เพิ่มเวลาโฟกัส
                totalFocusSeconds += diffSeconds;
                // ป้องกันพลังงานลดตอนจอดับ
                if (periodEnergy < 100) periodEnergy += 0.1;
            }
...
function startGameLoop() {
    ...
    gameInterval = setInterval(async () => {
        ...
        if (timeLeft > 0) {
            ...
            if (!isBreakMode) {
                totalFocusSeconds++;
                // เปิดหน้าจอค้างไว้ พลังงานค่อยๆ เพิ่มขึ้น
                if (periodEnergy < 100) periodEnergy += 0.2;
            }
            updateUI();
        } else {
...
// ✨ [หัวใจสำคัญ: ปรับจูนการตรวจจับจอดับ]
...
document.addEventListener('visibilitychange', () => {
    ...
    if (document.hidden) {
        ...
        // ⚡️ ขยายเกณฑ์เป็น 1000ms (1 วินาที)
        // ถ้าวินาทีที่หายไปสั้นกว่า 1 วิ = "สลับแอป" (เพราะนิ้วคนปัดเร็ว)
        // ถ้าวินาทีที่หายไปนานกว่า 1 วิ = "จอดับ" (เพราะระบบ OS ใช้เวลาประมวลผลการดับจอนานกว่า)
